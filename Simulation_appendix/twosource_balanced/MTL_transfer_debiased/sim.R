...
#lambda1_list = rep(NA, nreplications) ; lambda2_list = rep(NA, nreplications) ; lambda3_list = rep(NA, nreplications) 
lambda_debias_list = rep(NA, nreplications)
load('../MTL_transfer/summary.Rdata')
...
iter_vec = rep(NA, nreplications) ; runtime_vec = rep(NA, nreplications)
...
for(j in 1:nreplications){ # replication loop
  ...
  iter_vec[j] = admmm_mtl$iter ; runtime_vec[j] = admmm_mtl$runtime
  
  
  
  
  
  save(parameter_list, estimation_msevec, estimation_mse_bytask_list, prediction_msevec, prediction_mse_bytask_list, lambda_debias_list,
       
       
       iter_vec, runtime_vec,
