library(dplyr)
settingvec = c('threesource_balanced','threesource_aligned')              
...
for(i in 1:length(settingvec)){
  for(j in 1:length(methodvec)){
    ...
    msemat[length(methodvec)*(i-1)+j, 1:20] = log(p*estimation_msevec[1:20]^2)          
    predmat[length(methodvec)*(i-1)+j, 1:20] = prediction_msevec[1:20]                  
