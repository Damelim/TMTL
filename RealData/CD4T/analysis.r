...
num_common_features_target = rep(NA, 20)
num_common_features_debiased =rep(NA,20)
...
for(k in 1:20){
  ...
  Bmerged_list = list()
  for(j in 1:8){
    load(paste('MTL_notransfer_merged/summary_logtransform_target',j,'.Rdata',sep=""))
    Bmerged_list[[j]] = parameter_list[[k]] %>% as.matrix
  }
  Bdebiased_list = list()
  for(j in 1:8){
    load(paste('MTL_transfer_debiased/summary_logtransform_target',j,'.Rdata',sep=""))
    Bdebiased_list[[j]] = W_opt_debiased_list[[k]] ; rownames(Bdebiased_list[[j]]) = rownames(Bmerged_list[[1]])
  }
  Btarget_list = list()
  for(j in 1:8){
  ...
  for(j in 1:8){
    ...
    top_features_target[[j]] = names(sort(round(apply(Btarget_list[[j]], 1, l2norm),3), decreasing = T)[1:100])
    top_features_debiased[[j]] = names(sort(round(apply(Bdebiased_list[[j]], 1, l2norm),3), decreasing = T)[1:100])
  }
