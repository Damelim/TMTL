...
load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg1.Rdata")
X1 = X ; Y1 = Y %>% as.matrix() ; #Y1 = as.matrix(log1p(Y1)) #; X1 = as(X1, "dgCMatrix") ; Y1 = as(Y1, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg2.Rdata")
X2 = X ; Y2 = Y %>% as.matrix(); #Y2 = as.matrix(log1p(Y2)) #; X2 = as(X2, "dgCMatrix") ; Y2 = as(Y2, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg3.Rdata")
X3 = X ; Y3 = Y %>% as.matrix() ; #Y3 = as.matrix(log1p(Y3)) #; X3 = as(X3, "dgCMatrix") ; Y3 = as(Y3, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg4.Rdata")
X4 = X ; Y4 = Y %>% as.matrix() ; #Y4 = as.matrix(log1p(Y4)) #; X4 = as(X4, "dgCMatrix") ; Y4 = as(Y4, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg5.Rdata")
X5 = X ; Y5 = Y %>% as.matrix(); #Y5 = as.matrix(log1p(Y5)) #; X5 = as(X5, "dgCMatrix") ; Y5 = as(Y5, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg6.Rdata")
X6 = X ; Y6 = Y %>% as.matrix() ; #Y6 = as.matrix(log1p(Y6)) #; X6 = as(X6, "dgCMatrix") ; Y6 = as(Y6, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg7.Rdata")
X7 = X ; Y7 = Y %>% as.matrix() ; #Y7 = as.matrix(log1p(Y7)) #; X7 = as(X7, "dgCMatrix") ; Y7 = as(Y7, "dgCMatrix")

load("~/research_multitask/dataset_Batch1/clr_data_donor_nofiltered_1000hvg8.Rdata")
...
for(j in 1:nrep){
  ...
  #### this part changes ####
  X0_split_data = split_data(X1, Y1, train_prop, val_prop, test_prop, seed = j-1+nrep*0)
  X1_split_data = split_data(X2, Y2, train_prop, val_prop, test_prop, seed = j-1+nrep*1)
  X2_split_data = split_data(X3, Y3, train_prop, val_prop, test_prop, seed = j-1+nrep*2)
  X3_split_data = split_data(X4, Y4, train_prop, val_prop, test_prop, seed = j-1+nrep*3)
  X4_split_data = split_data(X5, Y5, train_prop, val_prop, test_prop, seed = j-4+nrep*4)
  X5_split_data = split_data(X6, Y6, train_prop, val_prop, test_prop, seed = j-4+nrep*5)
  X6_split_data = split_data(X7, Y7, train_prop, val_prop, test_prop, seed = j-4+nrep*6)
  ...
  save(prediction_mse_vec, W_opt_fused_list, file = paste("~/research_multitask/results_Batch1/summary_logtransform_target",tgtidx,".Rdata",sep=""))
}# end iteration loop
