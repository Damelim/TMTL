...
sourceCpp('~/research_multitask/MatMult_mac.cpp')
...
admm_stl_twosource <- function(X0, y0, X1, y1, X2, y2,
  ...
  for (iter in 1:max_iter) {
    ...
    objvalue[iter] = 1/(2*N)*(l2norm_sq(y0 - X0 %*% B0) + l2norm_sq(y1 - X1 %*% B1) + l2norm_sq(y2 - X2 %*% B2)) + lambda0 * sum(abs(Gamma00)) + lambda1 * sum(abs(Gamma1-Gamma00)) + lambda2 * sum(abs(Gamma2-Gamma00)) # objective value
