# settingvec = c('twosource_noskew','twosource_bigskew') #,'twosource_verybigskew')
# 
...
#         col = colvec, main = "No skew", xaxt = "n", yaxt = "n", outline = F, cex.main = 1, ylim = ylim_mtlresearch)
# axis(2)
# 
# boxplot(t(msemat[1*length(methodvec)+1:length(methodvec),]),
#         col = colvec, main = "Big skew", xaxt = "n", yaxt = "n", outline = F, cex.main = 1, ylim = ylim_mtlresearch)
# 
...
#     
#     load(paste(settingvec[i],'/',methodvec[j],'/summary.Rdata', sep = ""))
#     msemat[length(methodvec)*(i-1)+j, 1:3] = log(p*K*estimation_msevec^2)
#     
