...
#### Model 1) MTL with transfer (B^{debiased}) ####
...
lambda1_list = rep(NA, nreplications) ; lambda2_list = rep(NA, nreplications) ; lambda3_list = rep(NA, nreplications) ; lambda4_list = rep(NA, nreplications) ; lambda_debias_list = rep(NA, nreplications)
...
for(j in 1:nreplications){ # replication loop
  ...
  save(B0_list, B1_list, B2_list, B3_list, parameter_list, estimation_msevec, estimation_mse_bytask_list, prediction_msevec, prediction_mse_bytask_list, lambda1_list, lambda2_list, lambda3_list, lambda4_list, lambda_debias_list,
       file = "summary.Rdata")
}
