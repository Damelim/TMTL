...
set.seed(1)
L=3
p = 200 ; K = 10 ; n_source = 500 ; n_target = 100 ; N = L*n_source + n_target
lambda_values = 10^seq(-5, -1, length = 10) 
cv_folds = 3
...
for(i in 1:length(active_features_source)){
  Delta_W[i,] = rnorm(K, mean = alpha*Wmean/(sqrt(i)), sd = alpha*Wmean/(sqrt(i))) 
...
W_true_source1 = W_true_target + 3/4*Delta_W
W_true_source2 = W_true_target + 3/4*Delta_W
W_true_source3 = W_true_target - 3/2*Delta_W
